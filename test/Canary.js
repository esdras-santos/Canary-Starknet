...
describe('Canary protocol test', async function(){
    ...
    async function init() {
      ...
      owner = accounts[0]
      const Canary = await ethers.getContractFactory('Canary')
      canary = await Canary.deploy(owner.address)
      await canary.deployed()
      
      const CanaryToken = await ethers.getContractFactory('CanaryToken')
      canaryToken = await CanaryToken.deploy(owner.address, canary.address)
      await canaryToken.deployed()
      
      const Dungeon = await ethers.getContractFactory('Dungeon')
      collection = await Dungeon.deploy()
      await canaryToken.deployed()
      
      let tx

      tx = await canary.setGovernanceToken(canaryToken.address)
      await tx.wait()
      ...
      tx = await collection.approve(canary.address, '0')
      await tx.wait()
      tx = await collection.approve(canary.address, '1')
      await tx.wait()
      tx = await collection.approve(canary.address, '2')
      await tx.wait()

      tx = await canary.depositNFT(collection.address, '0', '3000000000000000', '30', '10')
      await tx.wait()
      tx = await canary.depositNFT(collection.address, '1', '9000000000000000', '30', '10')
      await tx.wait()
      tx = await canary.depositNFT(collection.address, '2', '20000000000000000', '30', '2')
      await tx.wait()
